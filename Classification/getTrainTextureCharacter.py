...
with open('./traintext.txt','r',encoding = 'utf-8') as f:
    ...
    for fi in lines:
        if fi.split(",")[3] == '1  0  0':
            col('./split/VIA/{}.jpeg'.format(fi.split(',')[0]),fi,fi.split(',')[0])
            for i in range(3):
                col('./featureSVM/augimg/{}_{}.jpeg'.format(fi.split(',')[0],i),fi,"{}_{}".format(fi.split(',')[0],i))
        if fi.split(",")[3] == '0  1  0':
            col('./split/VIA/{}.jpeg'.format(fi.split(',')[0]),fi,fi.split(',')[0])
        if fi.split(",")[3] == '0  0  1':
            col('./split/VIA/{}.jpeg'.format(fi.split(',')[0]),fi,fi.split(',')[0])
...
f = codecs.open('./split/augtexture.csv','w','gbk')
writer = csv.writer(f)
