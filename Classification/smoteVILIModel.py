...
def evaluate_model(x_train, y_train, model,x_test,y_test):
    ...
    # Control output name
    k = 6
    plt.savefig('./featureSVM/smoteVILI/matrixoupt{}.jpeg'.format(k))
    plt.clf()

    y_test = np.reshape(y_test,(np.shape(y_test)[0],1))
    rows  = []
    rows = list(preds)+list(y_test)
    f = codecs.open('./featureSVM/smoteVILI/output{}.csv'.format(k),'w','gbk')
    writer = csv.writer(f)
    ...
    plt.savefig('./featureSVM/smoteVILI/roc{}.jpeg'.format(k))
    ...
    plt.savefig('./featureSVM/smoteVILI/recall{}.jpeg'.format(k))
...
clf = svm.SVC(C = 2,kernel = 'rbf', gamma = 0.2,decision_function_shape = 'ovr',probability = 
True)
