...
clf = svm.SVC(C = 4, kernel = 'rbf', gamma = 0.04, decision_function_shape = 'ovr',probability = True)
...
k = 6
plt.savefig('./featureSVM/svmVIA/matrixoupt{}.jpeg'.format(k))
plt.clf()
# Output value
y_test = np.reshape(y_test,(np.shape(y_test)[0],1))
rows  = []
rows = list(y_hat)+list(y_test)
f = codecs.open('./featureSVM/svmVIA/output{}.csv'.format(k),'w','gbk')
writer = csv.writer(f)
...
plt.savefig('./featureSVM/svmVIA/roc{}.jpeg'.format(k))
...
plt.savefig('./featureSVM/svmVIA/recall{}.jpeg'.format(k))