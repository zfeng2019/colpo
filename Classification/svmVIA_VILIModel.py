...
clf = svm.SVC(C = 0.5, kernel = 'poly', gamma = 0.28, decision_function_shape = 'ovr',probability = True)
...
k = 6
plt.savefig('./featureSVM/svmVIA_VILI/matrixoupt{}.jpeg'.format(k))
plt.clf()
# Output value
y_test = np.reshape(y_test,(np.shape(y_test)[0],1))
rows  = []
rows = list(y_hat)+list(y_test)
f = codecs.open('./featureSVM/svmVIA_VILI/output{}.csv'.format(k),'w','gbk')
writer = csv.writer(f)
...
plt.savefig('./featureSVM/svmVIA_VILI/roc{}.jpeg'.format(k))
...
plt.savefig('./featureSVM/svmVIA_VILI/recall{}.jpeg'.format(k))